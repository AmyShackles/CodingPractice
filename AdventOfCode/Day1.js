...
function parseFrequency(arr) {
  let array = arr.split("\n");
  let answer = 0;
  for (let i = 0; i < array.length; i++) {
    if (array[i].includes("+")) {
      array[i] = parseInt(array[i].substr(1));
    } else {
      array[i] = parseInt(-array[i].substr(1));
    }
    answer += array[i];
...
already been seen. In this example, the first frequency reached twice is 2.
...
function firstRepeat(input) {
  ...
  while (true) {
    for (let i = 0; i < array.length; i++) {
      ...
      frequency += parseInt(array[i]);
      if (hash[frequency] && hash[frequency] !== array[i]) {
        return frequency;
      } else {
        hash[frequency] = array[i];
      }
    }
  }
}

firstRepeat(puzzleInput);
